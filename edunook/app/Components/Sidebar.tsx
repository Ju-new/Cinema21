        ...
        {links.map((link) => (
          <li key={link.name} className="my-2 relative">
            <button
              ...
              className={clsx(
                'w-full text-left cursor-pointer font-bold p-2 rounded transition-colors duration-150 relative hover:text-[#6482AD]',
                {
            ...
            >
              {pathname === link.href && (
                <span className="absolute left-[-16px] top-0 h-full w-2 bg-[#7d98c3] rounded-r"></span>
              )}
              {link.name}
            </button>
          </li>
